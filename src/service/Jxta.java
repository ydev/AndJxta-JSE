...
public class Jxta implements PipeMsgListener {
	...
	public void start(String name, File cache, String user, String pass) {
		instanceName = name;
		cacheHome = cache;
		username = user;
		password = pass;
	...
	private void configureJXTA() {
		...
		/*
		 * { // for Android ConfigurationFactory.setHome(new File(cacheHome,
		 * instanceName)); ConfigurationFactory.setPeerID(peerId);
		 * ConfigurationFactory.setName(instanceName);
		 * ConfigurationFactory.setUseMulticast(true); try {
		 * ConfigurationFactory.setRdvSeedingURI(new URI(rdvlist));
		 * ConfigurationFactory.setRelaySeedingURI(new URI(rdvlist)); } catch
		 * (URISyntaxException e) { e.printStackTrace(); } ConfigurationFactory
		 * factory = ConfigurationFactory.newInstance();
			...
			public void run() {
				while (true) {
					...
					try {
						...
						System.out
								.print("send message ('file' for file-transfer): ");
